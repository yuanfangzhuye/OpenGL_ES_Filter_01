...
- (void)filterInit
{
    ...
    //将jpg图片转化为纹理图片
    GLuint textureID = [self createTextureWithImage:img];
    //设置纹理ID
...
- (GLuint)createTextureWithImage:(UIImage *)img
{
    //1.将 UIImage 转化为 CGImageRef
...
- (void)setupShaderProgramWithName:(NSString *)name {
    ...
    //3.获取 Position、Texture、TextureCoords 的索引位置
    ...
    //7.打开 textureCoordsSlot 属性并传递数据到 textureCoordsSlot(纹理坐标)
    glEnableVertexAttribArray(textureCoordsSlot);
    glVertexAttribPointer(textureCoordsSlot, 2, GL_FLOAT, GL_FALSE, sizeof(SenceVertex), NULL + offsetof(SenceVertex, textureCoord));
    
    //8.保存 program，界面销毁则释放
